...
from pygezet import gezet, CMD_HELP, tgbot
from pygezet.pyrogram import eod, eor
...
async def module_help(client: Client, message: Message):
    if tgbot:
        ...
        if args:
            ...
            sleep(3)
            await xnxx.edit(
                f"**[✧ gezet Ubot ✧](https://github.com/alvrnxz/Gzpyro-Bot)**\n"
                f"**߷ Plugins** `{len(CMD_HELP)}` **Modules**\n"
                f"**♕︎ Tuhan:** [{user.first_name}](tg://user?id={user.id})\n\n"
                f"**⍟**   {string}"
                f"\n\n☞  **Support** : @GzSupportGroup\n☞  **Notes** :  `{choice(hndlr)}help gcast` **Untuk Melihat Modules Lainnya**"
            )
